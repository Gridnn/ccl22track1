...
class CorrectionNetwork(torch.nn.Module, ModuleUtilsMixin):
    ...
    def forward(self,
        ...
        if embed is None:
            embed = self.embeddings(input_ids=input_ids)
        ...
        if text_labels is not None:
            loss_fct = nn.CrossEntropyLoss(ignore_index=self.tokenizer.pad_token_id)  # -100 index = padding token
    ...
    def load_from_transformers_state_dict(self, gen_fp):
        ...
        gen_state_dict = transformers.AutoModelForMaskedLM.from_pretrained(gen_fp).state_dict()
...
class SoftMaskedBert4Csc(nn.Module):
    def __init__(self, cfg, device, tokenizer):
        super().__init__()
        self.cfg = cfg
        self._device = device
        self.config = transformers.AutoConfig.from_pretrained(cfg.pretrained_model)
        self.detector = DetectionNetwork(self.config)
        self.tokenizer = tokenizer
        self.corrector = CorrectionNetwork(self.config, tokenizer, self._device)
        self.corrector.load_from_transformers_state_dict(self.cfg.pretrained_model)
    ...
    def forward(self,
        ...
        cor_out = self.corrector(input_ids=input_ids,
        ...
        det_labels = torch.zeros(input_ids.shape).to(self._device)
        for i in range(len(input_ids)):
            for j in range(len(input_ids[i])):
                if input_ids[i][j] != cor_labels[i][j]:
                    det_labels[i][j] = 1
