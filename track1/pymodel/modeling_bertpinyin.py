...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.pinyin_embeddings = nn.Embedding(30, 32)
        self.gru = nn.GRU(32, config.hidden_size)
    ...
    def forward(self,
        ...
        # ebd_pinyin = self.pinyin_embeddings(pinyin_ids)
        # batch_size = pinyin_ids.shape[0]
        # out_pinyin = self.gru(ebd_pinyin.view(-1, pinyin_ids.shape[2], 32).float())[0]
        # out_pinyin = torch.sum(out_pinyin, 1)
        # out_pinyin = out_pinyin.reshape(batch_size, -1, self.config.hidden_size)
...
class BertEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.pinyin_embeddings = torch.nn.Embedding(len(self.lines), config.hidden_size)
        # self.pinyin_embeddings = torch.nn.Embedding(30, 32)
        # self.LayerNorm = torch.nn.LayerNorm(self.config.hidden_size)
        self.conv1 = nn.Conv1d(in_channels=self.config.hidden_size,
                               out_channels=self.config.hidden_size,
                               kernel_size=3,
                               padding=1)
        self.gru = nn.GRU(32, config.hidden_size)

    def forward_pinyin(self, pinyin_ids):
        # DCN(CNN)
        out_pinyin = self.pinyin_embeddings(pinyin_ids)
        out_pinyin = out_pinyin.permute(0, 2, 1)
        out_pinyin = self.conv1(out_pinyin)
        out_pinyin = out_pinyin.permute(0, 2, 1)
        ...
        # out_pinyin = torch.zeros(pinyin_ids.shape[0], pinyin_ids.shape[1], self.config.hidden_size)
        # for n, i in enumerate(pinyin_ids):
        #     i = i.view(-1, 5, 1)
        #     out = self.gru(i.float())[0]
        #     out = torch.sum(out, 1)
        #     out_pinyin[n] = out
        return out_pinyin
    ...
    def forward(
        ...
        pinyin_states = self.forward_pinyin(pinyin_ids).to(hidden_states.device)
